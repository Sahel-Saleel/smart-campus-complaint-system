...
from utils.mailer import send_mail_async, mail_is_configured
...
def dispatch_otp_email(recipient_email, otp_code, user=None, subject="Your Campus Voice Login OTP"):
    ...
    try:
        ...
        if mail_is_configured():
            # Sent on a background thread so a blocked/slow SMTP server can never hang this request
            send_mail_async(msg, description=f"OTP email to {recipient_email}")
        else:
...
def dispatch_onboarding_complete_email(user):
    ...
    try:
        ...
        # Sent on a background thread so a blocked/slow SMTP server can never hang the password change request
        send_mail_async(msg, description=f"onboarding completion email to {user.email}")
        return True
    except Exception as e:
        logger.error(f"Failed to prepare onboarding completion email: {e}")
        return False
...
def login():
    ...
    # The upper-case fallback only exists so users can type their default (registration-number)
    # password in lower case on first login. Once a user has chosen their own password, it is
    # verified exactly as entered.
    matched_password = password if verify_password(password, user.password) else None
    if matched_password is None and user.password_changed_at is None and verify_password(password.upper(), user.password):
        matched_password = password.upper()
    password_ok = matched_password is not None
    ...
    if not is_bcrypt_hash(user.password):
        try:
            # Re-hash the exact value that matched the stored hash (not the raw input), otherwise a
            # lower-case entry of an upper-case default password would silently change the password.
            user.password = hash_password(matched_password)
    ...
    # Start from a clean session so nothing from a previous login survives
    session.clear()
    ...
    # Security login alert for management accounts. This is best-effort: it is skipped when SMTP is
    # not configured and otherwise sent on a background thread, so it can never block or break login.
    if user.role and user.role.lower() in ['admin', 'hod', 'principal'] and user.email and user.email.strip():
        try:
            ...
            try:
                ...
                html_body = None
                logger.warning(f"Login alert template error: {tmpl_err}")
            ...
            send_mail_async(msg, description=f"login alert to {user.email}")
        except Exception as e:
            logger.error(f"Failed to prepare login alert email: {e}")
