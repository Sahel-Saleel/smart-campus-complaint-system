...
There are no demo accounts. User accounts are managed inside the application:

- **Principal / HODs / department admins** are created once with `flask --app wsgi bootstrap-accounts`, which prints a random one-time password for each account it creates. Every account must complete first-login onboarding (own password + email).
- **Students and staff** are added by their HOD from **Users → Add New User**.
- `flask --app wsgi check-default-passwords` lists any account still using a default password.
