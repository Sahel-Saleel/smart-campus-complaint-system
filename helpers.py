...
# Every department name that is used for the same branch in this system. The HOD accounts use the
# long official names (e.g. 'Computer Science and Engineering') while DEPT_MAP above uses short ones,
# so an exact string comparison wrongly rejected valid students of the HOD's own branch.
DEPT_CODE_ALIASES = {
    ...
    'EC': {'electronics engineering', 'electronics and communication engineering'},
    'AD': {'artificial intelligence and data science', 'artificial intelligence and data science engineering'},
}

def get_branch_code_for_department(department):
    """Return the 2-letter branch code (CS, EC, ...) for a department name, or None if the
    department is not a student branch (e.g. Applied Science and Humanities, Hostel)."""
    if not department:
        return None
    normalized = ' '.join(department.replace('&', 'and').lower().split())
    for code, names in DEPT_CODE_ALIASES.items():
        if normalized in names:
            return code
    return None

def student_belongs_to_department(registration_number, department):
    """True if the branch code inside a registration number (SPT24CS001 -> CS) matches the
    department. Departments that are not a student branch accept any valid registration number."""
    dept_code = get_branch_code_for_department(department)
    if dept_code is None:
        return True
    return registration_number[-5:-3].upper() == dept_code
