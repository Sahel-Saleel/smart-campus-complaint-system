...
from utils.helpers import (
    ...
    flag_overdue_complaints, student_belongs_to_department
)
...
def manage_users():
    ...
    if request.method == 'POST':
        ...
        # Optional details the HOD may fill in (the user can still edit them at first login)
        full_name = request.form.get('full_name', '').strip()
        email = request.form.get('email', '').strip().lower()
        initial_password = request.form.get('initial_password', '')

        # Duplicate checks (username, registration number and email are unique in the users table)
        existing_user = User.query.filter(
            (db.func.lower(User.username) == identifier.lower()) |
            (db.func.lower(User.registration_number) == identifier.lower())
        ).first()
        if existing_user:
        ...
        if User.query.filter(db.func.lower(User.email) == email.lower()).first():
            flash(f'The email address "{email}" is already used by another account.', 'error')
            return redirect(url_for('admin.manage_users'))

        # Initial password: the registration number / staff ID (existing behaviour), or a password
        # chosen by the HOD. Either way the user must set their own password at first login.
        if initial_password:
            is_strong, strength_err = validate_password_strength(initial_password)
            if not is_strong:
                flash(f'Initial password: {strength_err}', 'error')
                return redirect(url_for('admin.manage_users'))
        hashed_password = hash_password(initial_password or identifier)
        ...
        if is_maintenance_dept:
            ...
            if submitted_role == 'student' or (not submitted_role and is_student_username(identifier)):
                if not is_student_username(identifier):
                    flash('Invalid Registration Number. Expected format: SPT24CS001 (SPT/LSPT + year + branch code + 3 digits).', 'error')
                    return redirect(url_for('admin.manage_users'))
                if not student_belongs_to_department(identifier, current_user.department):
                    flash(f'You can only add students of your own department. {identifier} belongs to '
                          f'{extract_department_from_username(identifier)}.', 'error')
                    return redirect(url_for('admin.manage_users'))
                new_role = 'student'
        ...
        new_user = User(
            ...
            active=True,
            password_changed_at=None  # forces first-login onboarding (own password + email)
        )
        ...
        try:
            ...
            db.session.rollback()
            logger.warning(f"Duplicate user rejected by database while adding {identifier}")
            flash(f'Could not add "{identifier}": a user with this ID or email already exists.', 'error')
            return redirect(url_for('admin.manage_users'))
        except Exception as e:
            db.session.rollback()
            logger.error(f"Error adding user {identifier}: {e}", exc_info=True)
            flash(f'Could not add "{identifier}" because of a server error. Please try again.', 'error')
            return redirect(url_for('admin.manage_users'))

        log_audit(current_user.id, 'ADD_USER', details=f"Added {new_role} {new_user.username} to {current_user.department}")
        if initial_password:
            flash(f'{new_role.capitalize()} {new_user.username} added successfully. They can log in with '
                  f'username {new_user.username} and the initial password you set, and will be asked to '
                  f'choose their own password on first login.', 'success')
        else:
            flash(f'{new_role.capitalize()} {new_user.username} added successfully. They can log in with '
                  f'{new_user.username} as both username and password, and will be asked to choose their '
                  f'own password on first login.', 'success')
        return redirect(url_for('admin.manage_users'))
