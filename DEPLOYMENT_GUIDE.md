...
## 🔐 Initial Accounts for College Handover

No passwords are published in this repository. After the first deployment on an empty database, run `flask --app wsgi bootstrap-accounts` once; it creates the Principal / HOD / department-admin accounts and prints a random one-time password for each. Hand these over privately. Each account must complete onboarding (own password + email) on first login. Existing accounts are never modified.
